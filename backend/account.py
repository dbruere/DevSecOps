...
def account(db):
    ...
    sql_all_users = text("SELECT nom, prenom, login, classes, matiere, role FROM users")
    ...
    if request.method == 'POST':
        ...
        # On les rejoint avec des virgules (donnera "1,3"). S'il n'y a rien, on met None.
        ...
        if nom == "" or prenom == "" or role == "" or password == "":
            return render_template('account.html', error="Veuillez remplir tous les champs")
            ...
            if prenom and nom and role and password: 
                ...
                try:
                    ...
                    return render_template('account.html', success=f"Compte {login} créé !", admin=user, users_list=all_users)
                except Exception as e:
                    db.session.rollback()
                    return render_template('account.html', error=f"Erreur DB : {str(e)}", admin=user, users_list=all_users)
            else:
                return render_template('account.html', error="Veuillez remplir tous les champs", admin=user, users_list=all_users)
