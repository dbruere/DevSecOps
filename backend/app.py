...
# Une vraie clé secrète est nécessaire pour les sessions (utiliser
# variables d'environnement en prod)
app.secret_key = os.environ.get(
    "FLASK_SECRET_KEY", "super-secret-key-devsecops"
)

# Connexion à MySQL via Docker (nom du service 'db')
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get(
    "DATABASE_URL", "mysql+pymysql://test:test@127.0.0.1:3306/intranet"
)
...
with app.app_context():
    ...
    if not User.query.first():
        ...
        db.session.add(
            User(
                login="prof",
                password=generate_password_hash("prof123", method="scrypt"),
                role="professeur",
                prenom="Jean",
                nom="Prof",
            )
        ...
        db.session.add(
            User(
                login="eleve",
                password=generate_password_hash("eleve123", method="scrypt"),
                role="etudiant",
                prenom="Alice",
                nom="Eleve",
            )
...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if "user_id" not in session:
            flash(
                "Veuillez vous connecter pour accéder à cette page", "danger"
            )
...
def accueil():
    ...
    if session.get("user_role") == "etudiant":
        ...
        notes_query = (
            Note.query.filter_by(etudiant_id=session.get("user_id"))
            .order_by(Note.date_ajout.desc())
            .limit(3)
            .all()
        )
        ...
        notes_query = (
            Note.query.order_by(Note.date_ajout.desc()).limit(3).all()
        )
    ...
    liste_actualites = [
        {
            "type": "urgence",
            "titre": "Fermeture du serveur de test",
            "desc": "Le serveur sera indisponible aujourd'hui de 13h à 14h pour une mise à jour.",
            "date": "Aujourd'hui, 09:00",
        },
        {
            "type": "info",
            "titre": "Nouveau TP en ligne",
            "desc": "Les supports de cours d'Administration Linux avancée sont maintenant disponibles.",
            "date": "Hier, 16:30",
        },
        {
            "type": "info",
            "titre": "Réunion des délégués",
            "desc": "Rappel : réunion demain à 10h en salle B201.",
            "date": "Aujourd'hui, 08:15",
        },
        {
            "type": "urgence",
            "titre": "Alerte de Sécurité",
            "desc": "Merci de mettre à jour les mots de passe de vos sessions ENT.",
            "date": "À l'instant",
        },
        {
            "type": "info",
            "titre": "Conférence DevSecOps",
            "desc": "L'inscription à la conférence de la semaine prochaine est ouverte.",
            "date": "Hier, 14:00",
        },
    ]
...
def admin_users():
    if request.method == "POST":
        ...
        if action == "create":
            ...
            matiere = (
                request.form.get("matiere") if role == "professeur" else None
            )
            ...
            if delete_user_id:
                ...
                if user and getattr(user, "login", "") != "admin":
                    ...
                    flash(
                        "Action non autorisée : Vous ne pouvez pas supprimer le Super Admin",
                        "error",
                    )
            ...
            for uid in user_ids:
                ...
                if password_val and password_val.strip() != "":
                    user.password = generate_password_hash(
                        password_val.strip(), method="scrypt"
                    )
...
def notes():
    if request.method == "POST":
        ...
        if action == "delete":
            ...
            if session.get("user_role") == "admin":
                ...
                print(
                    f"================ EDIT RECEIVED: {note_id}, {nouvelle_valeur} ================",
                    flush=True,
                )
    ...
    # Un étudiant ne voit que ses notes, un prof ou admin voit toutes les
    # notes (ou selon les règles précises)
    matieres = [
...
def evaluations():
    ...
    # Récupérer les classes disponibles pour les profs (à partir de tous les
    # étudiants ou des cours existants)
    ...
    if request.method == "POST" and role in ["professeur", "admin"]:
        ...
        if action == "create":
            ...
            if evaluation and (
                role == "admin" or evaluation.professeur == username
            ):
...
def edt():
    ...
    return render_template(
        "edt.html", role=session.get("user_role", "etudiant")
    )
...
def api_cours_detail(cours_id):
    ...
    if session.get(
        "user_role"
    ) == "professeur" and cours.professeur != session.get("username"):
        return (
            jsonify({"error": "Ce cours appartient à un autre professeur"}),
            403,
        )
...
def forbidden(e):
    # Log attempt here
    print(
        f"Tentative d'accès non autorisé par l'utilisateur {session.get('username')} - IP: {request.remote_addr}"
    )
    return (
        "<h1>403 Forbidden</h1><p>Vous n'avez pas l'autorisation d'accéder à cette ressource.</p>",
        403,
    )
